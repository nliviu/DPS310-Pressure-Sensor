 ...
 * @brief
 *
 * Temperature measurements must be enabled for the DPS422 to compensate for
 * temperature drift in the pressure measurement.
 * @file Dps422.h
...
class Dps422 : public DpsClass {
 public:
  ...
  /**
   * @brief measures both temperature and pressure values, when op mode is set
   * to CMD_BOTH
   *
   ...
   * @return status code
   */
  int16_t measureBothOnce(float &prs,
                          float &temp);  // might make sense to declare in base
                                         // class for future sensors

  int16_t measureBothOnce(float &prs, float &temp, uint8_t prs_osr,
                          uint8_t temp_osr);

 ...
 *
 * "Dps310" represents Infineon's high-sensetive pressure and temperature
 * sensor. It measures in ranges of 300 - 1200 hPa and -40 and 85 °C. The sensor
 * can be connected via SPI or I2C. It is able to perform single measurements or
 * to perform continuous measurements of temperature and pressure at the same
 * time, and stores the results in a FIFO to reduce bus communication.
 *
...
class DpsClass {
 public:
   ...
   */
  void begin(struct mgos_i2c *bus);
   ...
   */
  void begin(struct mgos_i2c *bus, uint8_t slaveAddress);
   ...
   * of measurements equals to 2^n, if the value written to the register field
   * is n. 2^n internal measurements are combined to return a more exact
   * measurement
   * @return 			status code
   */
   ...
   * @param oversamplingRate: 	DPS__OVERSAMPLING_RATE_1,
   * DPS__OVERSAMPLING_RATE_2, DPS__OVERSAMPLING_RATE_4 ...
   * DPS__OVERSAMPLING_RATE_128, which are defined as integers 0 - 7
   * @return 			status code
   */
  ...
  /**
   * starts a continuous temperature measurement with specified measurement rate
   * and oversampling rate If measure rate is n and oversampling rate is m, the
   * DPS310 performs 2^(n+m) internal measurements per second. The DPS310 cannot
   * operate with high precision and high speed at the same time. Consult the
   * datasheet for more information.
   *
   * @param measureRate: 		DPS__MEASUREMENT_RATE_1,
   * DPS__MEASUREMENT_RATE_2,DPS__MEASUREMENT_RATE_4 ...
   ...
   * @return 			status code
   */
  int16_t startMeasurePressureCont(uint8_t measureRate,
                                   uint8_t oversamplingRate);

  /**
   * starts a continuous temperature and pressure measurement with specified
   * measurement rate and oversampling rate for temperature and pressure
   * measurement respectvely.
   *
   * @param tempMr				measure rate for temperature
   * @param tempOsr				oversampling rate for
   * temperature
   * @param prsMr				measure rate for pressure
   * @param prsOsr				oversampling rate for pressure
   * @return 			status code
   */
  int16_t startMeasureBothCont(uint8_t tempMr, uint8_t tempOsr, uint8_t prsMr,
                               uint8_t prsOsr);
   ...
   * 				0 if the FIFO is not full or FIFO interrupt is
   * disabled -1 on fail
   */
  int16_t getIntStatusFifoFull(void);

  /**
   * Gets the interrupt status flag that indicates a finished temperature
   * measurement
   *
  ...
  /**
   * Gets the interrupt status flag that indicates a finished pressure
   * measurement
   *
 ...
 protected:
   ...
   *
   * @param temp_mr: 		DPS__MEASUREMENT_RATE_1,
   * DPS__MEASUREMENT_RATE_2,DPS__MEASUREMENT_RATE_4 ...
   * DPS__MEASUREMENT_RATE_128
   * @param temp_osr: 	DPS__OVERSAMPLING_RATE_1, DPS__OVERSAMPLING_RATE_2,
   ...
   *
   * @param prs_mr: 		DPS__MEASUREMENT_RATE_1,
   * DPS__MEASUREMENT_RATE_2,DPS__MEASUREMENT_RATE_4 ...
   * DPS__MEASUREMENT_RATE_128
   * @param prs_osr: 	DPS__OVERSAMPLING_RATE_1, DPS__OVERSAMPLING_RATE_2,
   * DPS__OVERSAMPLING_RATE_4 ... DPS__OVERSAMPLING_RATE_128
   * @return 	0 normally or -1 on fail
   */
  ...
  /**
   * calculates the time that the sensor needs for 2^mr measurements with an
   * oversampling rate of 2^osr (see table "pressure measurement time (ms)
   * versus oversampling rate") Note that the total measurement time for
   * temperature and pressure must not be more than 1 second. Timing behavior of
   * pressure and temperature sensors can be considered the same.
   *
   * @param mr: 		DPS__MEASUREMENT_RATE_1,
   * DPS__MEASUREMENT_RATE_2,DPS__MEASUREMENT_RATE_4 ...
   * DPS__MEASUREMENT_RATE_128
   * @param osr: 	DPS__OVERSAMPLING_RATE_1, DPS__OVERSAMPLING_RATE_2,
   * DPS__OVERSAMPLING_RATE_4 ... DPS__OVERSAMPLING_RATE_128
   ...
   *
   * @param value: 	the raw pressure or temperature value read from the
   * pressure register blocks, where the LSB of PRS_B0 marks wheather the value
   * is a temperatur or a pressure.
   *
   * @return	-1 on fail
  ...
  /**
   * Gets the results from continuous measurements and writes them to given
   * arrays
   *
   * @param *tempBuffer: 	The start address of the buffer where the
  ...
  int16_t getContResults(float *tempBuffer, uint8_t &tempCount,
                         float *prsBuffer, uint8_t &prsCount, RegMask_t reg);
   ...
   * @return 	number of bytes that have been read successfully, which might
   * not always equal to length due to rx-Buffer overflow etc.
   */
  ...
  /**
   * @brief converts non-32-bit negative numbers to 32-bit negative numbers with
   * 2's complement
   *

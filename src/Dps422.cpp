...
int16_t Dps422::measureBothOnce(float &prs, float &temp, uint8_t prs_osr,
  ...
  switch (rdy) {
    ...
    case 1:  // measurement ready, expected case
      ...
      int32_t raw_temp;
      int32_t raw_psr;
      if (getRawResult(&raw_temp, registerBlocks[TEMP]) ||
...
int16_t Dps422::getContResults(float *tempBuffer, uint8_t &tempCount,
  ...
  return DpsClass::getContResults(tempBuffer, tempCount, prsBuffer, prsCount,
                                  registers[FIFO_EMPTY]);
}
...
int16_t Dps422::setInterruptSources(uint8_t intr_source, uint8_t polarity) {
  ...
  return writeByteBitfield(intr_source, registers[INTR_SEL]) ||
...
void Dps422::init(void) {
  // m_lastTempScal = 0.08716583251; // in case temperature reading disabled,
  // the default raw temperature value correspond the reference temperature of
  // 27 degress.
  standby();
...
int16_t Dps422::readcoeffs(void) {
  ...
  int32_t t_Vbe = ((uint32_t) buffer_temp[1] & 0x01) |
                  ((uint32_t) buffer_temp[2] << 1);  // 9 bits
  ...
  float k_ptat = (DPS422_V_BE_TARGET - Vbe_cal_tref) * DPS422_K_PTAT_CORNER +
                 DPS422_K_PTAT_CURVATURE;
  // 7. calculate A' and B'
  ...
  m_c00 = ((uint32_t) buffer_prs[0] << 12) | ((uint32_t) buffer_prs[1] << 4) |
          (((uint32_t) buffer_prs[2] & 0xF0) >> 4);
  m_c10 = ((uint32_t)(buffer_prs[2] & 0x0F) << 16) |
          ((uint32_t) buffer_prs[3] << 8) | (uint32_t) buffer_prs[4];
  m_c01 = ((uint32_t) buffer_prs[5] << 12) | ((uint32_t) buffer_prs[6] << 4) |
          (((uint32_t) buffer_prs[7] & 0xF0) >> 4);
  m_c02 = ((uint32_t)(buffer_prs[7] & 0x0F) << 16) |
          ((uint32_t) buffer_prs[8] << 8) | (uint32_t) buffer_prs[9];
  m_c20 = ((uint32_t)(buffer_prs[10] & 0x7F) << 8) | (uint32_t) buffer_prs[11];
  m_c30 = ((uint32_t)(buffer_prs[12] & 0x0F) << 8) | (uint32_t) buffer_prs[13];
  m_c11 = ((uint32_t) buffer_prs[14] << 9) | ((uint32_t) buffer_prs[15] << 1) |
          (((uint32_t) buffer_prs[16] & 0x80) >> 7);
  m_c12 = (((uint32_t) buffer_prs[16] & 0x7F) << 10) |
          ((uint32_t) buffer_prs[17] << 2) |
          (((uint32_t) buffer_prs[18] & 0xC0) >> 6);
  m_c21 =
      (((uint32_t) buffer_prs[18] & 0x3F) << 8) | ((uint32_t) buffer_prs[19]);
...
float Dps422::calcPressure(int32_t raw_prs) {
  ...
  prs = m_c00 + m_c10 * prs + m_c01 * temp + m_c20 * prs * prs +
        m_c02 * temp * temp + m_c30 * prs * prs * prs + m_c11 * temp * prs +
        m_c12 * prs * temp * temp + m_c21 * prs * prs * temp;
  return prs;
}
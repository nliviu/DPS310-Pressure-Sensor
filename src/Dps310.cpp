...
int16_t Dps310::getContResults(float *tempBuffer, uint8_t &tempCount,
...
int16_t Dps310::setInterruptSources(uint8_t intr_source, uint8_t polarity) {
  ...
  return writeByteBitfield(intr_source, registers[INT_SEL]) ||
         writeByteBitfield(polarity, registers[INT_HL]);
}
#endif
...
float Dps310::calcPressure(int32_t raw) {
  ...
  prs = m_c00 + prs * (m_c10 + prs * (m_c20 + prs * m_c30)) +

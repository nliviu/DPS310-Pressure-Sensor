...
int16_t DpsClass::getContResults(float *tempBuffer, uint8_t &tempCount,
...
int16_t DpsClass::startMeasureTempCont(uint8_t measureRate,
...
int16_t DpsClass::startMeasurePressureCont(uint8_t measureRate,
                                           uint8_t oversamplingRate) {
  // abort if initialization failed
  if (m_initFail) {
    return DPS__FAIL_INIT_FAILED;
  ...
  // update precision and measuring rate
  if (configPressure(measureRate, oversamplingRate)) return DPS__FAIL_UNKNOWN;
  // enable result FIFO
  if (enableFIFO()) {
    return DPS__FAIL_UNKNOWN;
  }
...
int16_t DpsClass::startMeasureBothCont(uint8_t tempMr, uint8_t tempOsr,
                                       uint8_t prsMr, uint8_t prsOsr) {
  // abort if initialization failed
  if (m_initFail) {
    return DPS__FAIL_INIT_FAILED;
  }
  ...
  // abort if speed and precision are too high
  if (calcBusyTime(tempMr, tempOsr) + calcBusyTime(prsMr, prsOsr) >=
      DPS310__MAX_BUSYTIME) {
    return DPS__FAIL_UNFINISHED;
  }
  // update precision and measuring rate
  ...
  // update precision and measuring rate
  if (configPressure(prsMr, prsOsr)) return DPS__FAIL_UNKNOWN;
  // enable result FIFO
  if (enableFIFO()) {
    return DPS__FAIL_UNKNOWN;
  }
...
int16_t DpsClass::getFIFOvalue(int32_t *value) {
  ...
  if (value == NULL ||
      readBlock(registerBlocks[PRS], buffer) != DPS__RESULT_BLOCK_LENGTH)
    return DPS__FAIL_UNKNOWN;
...
int16_t DpsClass::readByteSPI(uint8_t regAddress) {
  ...
  m_spibus->beginTransaction(
      SPISettings(DPS310__SPI_MAX_FREQ, MSBFIRST, SPI_MODE3));
...
int16_t DpsClass::readBlockSPI(RegBlock_t regBlock, uint8_t *buffer) {
  ...
  for (uint8_t count = 0; count < regBlock.length; count++) {
    buffer[count] =
        m_spibus->transfer(0xFF);  // send a dummy byte while receiving
  }
...
#ifndef DPS_DISABLESPI
int16_t DpsClass::writeByteSpi(uint8_t regAddress, uint8_t data,
                               uint8_t check) {
  // this function is only made for communication via SPI
  if (m_SpiI2c != 0) {
    return DPS__FAIL_UNKNOWN;
...
int16_t DpsClass::writeByteBitfield(uint8_t data, RegMask_t regMask) {
  return writeByteBitfield(data, regMask.regAddress, regMask.mask,
                           regMask.shift, 0U);
}

int16_t DpsClass::writeByteBitfield(uint8_t data, uint8_t regAddress,
